...
{
	return syscall(syscall_write, (uint32_t)fd, (uint32_t)buf, len, 0, 0); //4 means write
}
...
{
	syscall(syscall_sleep, time, 0, 0, 0, 0);
}
...
{
	return syscall(syscall_sem_init, (uint32_t)sem, value, 0, 0, 0);	
}

int sem_post(sem_t *sem)
{
	return syscall(syscall_sem_post, (uint32_t)sem, 0, 0, 0, 0);
}

int sem_wait(sem_t *sem)
{
	return syscall(syscall_sem_wait, (uint32_t)sem, 0, 0, 0, 0);
}

int sem_destroy(sem_t *sem)
{
	return syscall(syscall_sem_destroy, (uint32_t)sem, 0, 0, 0, 0);
}
